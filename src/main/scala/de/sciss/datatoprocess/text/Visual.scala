...
//    def noise_=(value: Int): Unit = noiseOp.setAmount(value)
    ...
    private def setText1(value: String): Unit =
      visDo {
        stopAnimation()
        setText(value)
        startAnimation()
...
//      )
    ...
    private def execOnEDT[A](code: => A): A = {
      val p = Promise[A]()
      Swing.onEDT {
        val res = Try {
          visDo(code)
    ...
    def saveFrameSeriesAsPNG(baseFile: File, numFrames: Int, anim: Anim, skipFrames: Int): Processor[Unit] = {
      ...
      Processor[Unit]("saveFrameSeriesAsPNG") { p =>
        ...
        def mkF() = parent / f"$child-${frame - skipFrames + 1}%d.png"
        ...
        implicit val rnd: util.Random = new util.Random(0L)
