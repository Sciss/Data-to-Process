...
object Text {
      ...
      def apply(): Unit = {
        ...
        FileDialog.save(init = Some(userHome / s"text_$initName.png")).show(None).foreach { f =>
          ...
          if (optMovie.show(title = "Image Sequence Settings") == OptionPane.Result.Ok) {
            ...
            val MovieConfig(duration, fps, skipFrames) = pMovie.value
            val numFrames = (duration * fps + 0.5).toInt
            val pFull     = renderImageSequence(v, anim, numFrames = numFrames, f = f, skipFrames = skipFrames)
  ...
  def renderImageSequence(v: text.Visual, anim: Anim, numFrames: Int, f: File, skipFrames: Int): Processor[Unit] =
    v.saveFrameSeriesAsPNG(baseFile = f, numFrames = numFrames, anim = anim, skipFrames = skipFrames)
}